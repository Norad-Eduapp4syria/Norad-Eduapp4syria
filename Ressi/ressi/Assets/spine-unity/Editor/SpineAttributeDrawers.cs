...
namespace Spine.Unity.Editor {
	...
	public abstract class SpineTreeItemDrawerBase<T> : PropertyDrawer where T:SpineAttributeBase {
        ...
        SkeletonDataAsset GetSkeletonFromRessi(Rect position)
        {
            var ressi = GameObject.FindGameObjectWithTag(Constants.Tags.Player);
            if(ressi == null)
            {
                EditorGUI.LabelField(position, "ERROR:", "No GameObject with " + Constants.Tags.Player + " tag");
            }
            var skeletonRenderer = ressi.GetComponentInChildren<SkeletonRenderer>();
		...
		public override void OnGUI (Rect position, SerializedProperty property, GUIContent label) {
            ...
            if(skeletonDataAsset == null)
                skeletonDataAsset = GetSkeletonFromRessi(position);
